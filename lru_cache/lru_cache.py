...
class LRUCache:
    ...
    can hold(i.e. limit), the current number of nodes it is holding, a doubly-
    ...
    def set(self, key, val ):
        # if key already stored , overwrite the old value
        if key in self.dict:
            # overwrite in dictionary
            self.dict[key]=val
            # overwrite in dll
            #  ie iterate across and find the node to be updated
            node=self.dll.head
            while node is not None:
                # check key equality
                if key  == node.value[0]:
